...
class BaseBithumb(object):
    ...
    def _sign_generator(self, *args):
        ...
        hashed = hmac.new(
            self._secret.encode("utf-8"), hmac_data.encode("utf-8"), hashlib.sha512
        ).hexdigest()
        ...
        try:
            ...
            if status and status == "0000":
                ...
                message = response.get(
                    "message", WarningMsg.MESSAGE_NOT_FOUND.format(name=self.name)
                )
            ...
            return ExchangeResult(
                False,
                message=WarningMsg.EXCEPTION_RAISED.format(name=self.name),
                wait_time=1,
            )
    ...
    def withdraw(self, coin, amount, to_address, payment_id=None):
        params = {"currency": coin, "units": amount, "address": to_address}
        if payment_id:
    ...
    def check_order(self, data, profit_object):
        ...
        result = self._private_api(
            Consts.POST,
            Urls.ORDER_DETAIL,
            dict(order_id=order_id, order_currency=alt, payment_currency=base),
        )
    ...
    async def _get_deposit_addrs(self, currency):
        for _ in range(3):
            result_object = await self._async_private_api(
                Consts.POST, Urls.DEPOSIT_ADDRESS, {"currency": currency}
            )
    ...
    async def _get_balance(self):
        for _ in range(3):
            result_object = await self._async_private_api(
                Consts.POST, Urls.BALANCE, {"currency": "ALL"}
            )
            if result_object.success:
                break
    ...
    async def _get_trading_fee(self, symbol):
        for _ in range(3):
            result_object = await self._async_private_api(
                Consts.POST, Urls.ACCOUNT, {"currency": symbol}
            )
            if result_object.success:
                break
    ...
    async def get_balance(self):
        ...
        if result_object.success:
            ...
            response = {
                key.split("_")[1].upper(): float(data["data"][key])
                for key in data["data"]
                if key.startswith("available") and float(data["data"][key]) > 0
            }
    ...
    async def get_transaction_fee(self):
        ...
        try:
            ...
            for tag in tags[3:]:
                key = re.search(
                    "[A-Z]+", tag.cssselect("td.money_type")[0].text_content()
                ).group()
                try:
            ...
            if ret_data is dict():
                return ExchangeResult(
                    False,
                    message=WarningMsg.TRANSACTION_FAILED.format(name=self.name),
                    wait_time=5,
                )
            else:
    ...
    async def get_deposit_addrs(self, coin_list=None):
        ...
        for currency in AVAILABLE_COINS:
            ...
            if deposit_result_object.success:
                ...
                failed_log += "[{}], {} \n".format(
                    currency, deposit_result_object.message
                )
    ...
    async def get_curr_avg_orderbook(self, currencies, default_btc=1):
        ...
        for order_type in [Consts.BIDS, Consts.ASKS]:
            ...
            btc_average[order_type] = (total_price / total_amount).quantize(
                Decimal(10) ** -8
            )
            ...
            for order_type in [Consts.BIDS, Consts.ASKS]:
                ...
                for row in rows:
                    if order_type == Consts.BIDS:
                        total_price += (
                            Decimal(row["price"])
                            / btc_average[Consts.ASKS]
                            * Decimal(row["quantity"])
                        )
                    else:
                        total_price += (
                            Decimal(row["price"])
                            / btc_average[Consts.BIDS]
                            * Decimal(row["quantity"])
                        )
                    total_amount += Decimal(row["quantity"])
                ...
                avg_orderbook[sai_coin][order_type] = (
        ...
        for _ in range(3):
            ...
            if success:
                ...
                for currency_pair in currency_pairs:
                    ...
                    m_to_s[currency_pair] = float(
                        ((s_bid - m_ask) / m_ask).quantize(Decimal(10) ** -8)
                    )

                    m_bid = bithumb_result_object.data[currency_pair][Consts.BIDS]
                    s_ask = other_result_object.data[currency_pair][Consts.ASKS]
                    s_to_m[currency_pair] = float(
                        ((m_bid - s_ask) / s_ask).quantize(Decimal(10) ** -8)
                    )

                orderbooks = (
                    bithumb_result_object.data,
                    other_result_object.data,
                    {
                        Consts.PRIMARY_TO_SECONDARY: m_to_s,
                ...
                await asyncio.sleep(
                    max(bithumb_result_object.wait_time, other_result_object.wait_time)
                )
        else:
            error_message = (
                bithumb_result_object.message + "\n" + other_result_object.message
            )
