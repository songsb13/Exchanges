...
class Korbit:
    ...
    def base_to_alt(self, currency_pair, btc_amount, alt_amount, td_fee, tx_fee):
        ...
        return False, '', 'Korbit에서는 BTC만 출금이 가능합니다.', 0
    ...
    async def get_transaction_fee(self):# async
        ...
        transaction_fee = {
            'BTC': Decimal(0.001).quantize(Decimal(10)**-8,rounding=ROUND_DOWN),
            # 'BCH': 0.0005,
            # 'ETH': 0.01,
            # 'ETC': 0.01,
            # 'XRP': 0.2,
    ...
    async def get_curr_avg_orderbook(self, coin_name,btc_sum = 1):
        ...
        try:
            for _currency_pair in ['BTC_BTC']+coin_name:
                ...
                while True:
                    orderbook = self.public_api('GET', '/v1/orderbook',
                                            {'currency_pair': _currency_pair.split('_')[1].lower() + '_krw'})
                    _stat = self.error_check(orderbook, 10)
                ...
                for order_type in ['asks', 'bids']:
                    ...
                    _order_sum = 0
                    btc_amount = 0
                    _list = orderbook[order_type]
                    # _list.reverse()

                    for _info in _list:
                        if _currency_pair == 'BTC_BTC':
                            _order_amount += Decimal(_info[1])
                            btc_amount += Decimal(_info[1])
                        elif order_type == 'asks':
                            _order_amount += Decimal(_info[1])
                            btc_amount += Decimal(_info[1]) * (Decimal(_info[0])/btc_avg_list[1])
                        elif order_type == 'bids':
                            _order_amount += Decimal(_info[1])
                            btc_amount += Decimal(_info[1]) * (Decimal(_info[0])/btc_avg_list[0])
                        _order_sum += (Decimal(_info[0]) * Decimal(_info[1])).quantize(Decimal(10) ** -8)

                        if btc_amount >= btc_sum:
