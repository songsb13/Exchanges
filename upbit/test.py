...
class TestBaseUpbit(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        exchange = upbit.BaseUpbit(key="", secret="")
...
class TestTradeMarket(TestBaseUpbit):
    def test_under_minimum_buy(self):
        ...
        trading_result = self.exchange.buy(
    ...
    def test_under_minimum_sell(self):
        ...
        amount = Decimal(krw_btc_minimum_price / krw_btc_price).quantize(
            Decimal(10) ** -8, rounding=ROUND_DOWN
        )

        trading_result = self.exchange.sell(
    ...
    def test_over_balance_buy(self):
        ...
        trading_result = self.exchange.buy(
            self.test_main_sai_symbol, BaseTradeType.BUY_MARKET, price=krw_over_balance
        )
        self.assertFalse(trading_result.success)
    ...
    def test_over_balance_sell(self):
        ...
        amount = Decimal(krw_over_balance / krw_btc_price).quantize(
            Decimal(10) ** -8, rounding=ROUND_DOWN
        )
    ...
    def test_no_amount_buy(self):
        empty_price = 0
        trading_result = self.exchange.buy(
            self.test_main_sai_symbol, BaseTradeType.BUY_MARKET, price=empty_price
        )
    ...
    def test_no_amount_sell(self):
        empty_amount = 0
        trading_result = self.exchange.sell(
            self.test_main_sai_symbol, BaseTradeType.SELL_MARKET, amount=empty_amount
        )
        self.assertFalse(trading_result.success)
    ...
    def test_incorrect_lot_size_buy(self):
        ...
        trading_result = self.exchange.buy(
            self.test_main_sai_symbol, BaseTradeType.BUY_MARKET, price=trading_price
        )
