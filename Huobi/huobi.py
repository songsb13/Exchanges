...
class Huobi(BaseExchange):
    ...
    def _get_account_id(self):
        for _ in range(10):
            id_suc, id_data, id_msg, id_time = self._private_api('GET', '/v1/account/accounts')

            if id_suc:
                self._account_id = id_data['data'][0]['id']
    ...
    async def get_deposit_addrs(self, coin_list=None):
        gac_suc, gac_data, gac_msg, gac_times = self._currencies()
        ...
        if gac_suc:
            try:
                ...
                for coin in coins:
                    s, data, m, t = await self._async_private_api("GET", '/v1/query/deposit-withdraw',
    ...
    async def get_balance(self):
        if self._account_id is None:
            ...
            if not id_s:
                return False, '', id_m, id_t

        s, data, m, t = await self._async_private_api(
        ...
        if s:
            ...
            return s, balance, m, t

        else:
            return False, '', '[Huobi]지갑 값을 가져오는데 실패했습니다. [{}]'.format(m), 5
    ...
    async def get_curr_avg_orderbook(self, coin_list, btc_sum=1):  # 상위 평균매도/매수가 구함
        ...
        for currency_pair in coin_list:
            ...
            s, data, m, t = await self._async_public_api('GET', self._base_url + '/market/depth',
...
#         debugger.info('### buy [{}],[{}] 입력됨'.format(coin, amount))
#
#         coin = coin.split('_')
#         currency_pair = coin[1].lower() + coin[0].lower()
#
...
#
#             if suc:
#                 debugger.debug('AltToBase 거래에 성공했습니다.')
#
#                 return suc, data, msg, times
#
#             else:
...
#
#     def withdraw(self, coin, amount, to_address, payment_id=None):
#         debugger.info(('출금, Coin-[{}] Amount-[{}] ToAddress-[{}] PaymentId-[{}] '
#                        '입력되었습니다.').format(coin, amount, to_address, payment_id))
#
#         params = {
#                     'currency': coin.lower(),
#                     'address': to_address,
...
#
#         if not suc:
#             debugger.debug('출금 값을 가져오지 못했습니다. [{}]'.format(msg))
#
...
#
#     def get_available_coin(self):
#         suc, data, msg, times = self.http_request('GET', self._base_url + '/v1/common/currencys')
#
#         if not suc:
#             debugger.debug('사용가능한 코인 값을 가져오지 못했습니다. [{}]'.format(msg))
#
#             return False, '', '[Huobi]사용가능한 코인 값을 가져오지 못했습니다. [{}]'.format(msg), 1
#
#         else:
...
#
#     async def _async_public_api(self, method, path, params=None):
#         sign_data = {
#                     'AccessKeyId': self._key,
#                     'SignatureMethod': 'HmacSHA256',
#                     'SignatureVersion': self.signature_version,
...
#             url += '?' + urlencode(sign_data)
#
#         suc, data, msg, times = await self.async_http_request(method, url, params)
#
#         return suc, data, msg, times
#
#
...
#                                                                              self._base_url + '/market/history/trade',
#                                                                              {'symbol': currency_pair})
#                     if suc:
...
#         # Binance는 trade_fee가 0.1로 되어있음.
#         # https://www.huobi.pro/ko-kr/about/fee/ 에 가보면 모든값이 0.2%의 거래수수료를 받는다.
#         return True, 0.002, '', 0
#
...
#                             s, "GET", '/v1/query/deposit-withdraw',
#                             {"currency": coin, "type": "deposit", "from": "0", "size": "100"}
#                         )
...
#
#                                 if (coin_info['currency'] == 'xrp'
#                                         or coin_info['currency'] == 'xmr'
#                                         or coin_info['currency'] == 'eos'):
...
#
#             suc, data, msg, times = await self.async_api_request(
#                 sync, 'GET', '/v1/account/accounts/{}/balance'.format(self.account_id), params)
#
#             if suc:
#                 balance = {}
#                 for info in data['data']['list']:
...
#
#             else:
#                 return False, '', '[Huobi]지갑 값을 가져오는데 실패했습니다. [{}]'.format(msg), 5
#
#     async def get_curr_avg_orderbook(self, coin_list, btc_sum=1):  # 상위 평균매도/매수가 구함
#         avg_order_book = {}
#         async with aiohttp.ClientSession(headers=self.get_headers) as sync:
...
#                     suc, data, msg, times = await self.async_http_request(
#                         sync, 'GET', self._base_url + '/market/depth', params)
#
#                     if suc:
#                         book = data['tick']
#                         for types in ['asks', 'bids']:
...
#                     else:
#                         return (False, '',
#                                  '[Huobi]마켓의 과거 코인가격을 가져오는데 실패했습니다. [{}]'.format(msg),
#                                  times)
#
#                 return True, avg_order_book, '', 0
#
